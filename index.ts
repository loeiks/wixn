...
const main = async () => {
  try {
    ...
    const summary = chalk.bold(
      `${wixn.action.slice(0, -4)}led Packages: ${chalk.green(completedPackages.join(", "))} ${chalk.bold((end / 1000).toFixed(2))}s`,
    );
